...
const groupedData = computed(() => {
    ...
    if (activeTab.value === 'classes') {
        return classes.map(cls => {
            ...
            return {
                ...
                stats: [
                    { icon: 'ri-book-read-line', text: `${classLessons.length} dars` },
                    { icon: 'ri-time-line', text: `${totalLessonsCount} jami dars` },
                    { icon: 'ri-graduation-cap-line', text: `${uniqueTeachers} o'qituvchi` },
                    { icon: 'ri-file-text-line', text: `${uniqueSubjects} fan` }
                ],
                items: classLessons,
    ...
    if (activeTab.value === 'teachers') {
        return teachers.map(t => {
            ...
            const totalLessonsCount = teacherLessons.reduce((sum, l) => sum + (l.lessonCount || 0), 0);
            const uniqueClasses = new Set(teacherLessons.map(l => l.classId).filter(id => id)).size;
            
            return {
                id: t.id, title: t.fullName,
                stats: [
                    { icon: 'ri-group-line', text: `${uniqueClasses} sinf` },
                    { icon: 'ri-book-read-line', text: `${teacherLessons.length} dars` },
                    { icon: 'ri-time-line', text: `${totalLessonsCount} jami dars` }
                ],
                items: teacherLessons,
                addText: "O‘qituvchi uchun dars qo‘shish",
                editText: "O‘qituvchi tahrirlash"
            };
        });
    }
        ...
        return subjects.map(s => {
            ...
            return {
                ...
                stats: [
                    { icon: 'ri-graduation-cap-line', text: `${uniqueTeachers} o'qituvchi` },
                    { icon: 'ri-group-line', text: `${uniqueClasses} sinf` },
                    { icon: 'ri-book-read-line', text: `${subjectLessons.length} dars` }
                ],
    ...
    if (activeTab.value === 'rooms') {
        return rooms.map(r => {
            ...
            const totalLessonsCount = roomLessons.reduce((sum, l) => sum + (l.lessonCount || 0), 0);
        ...
        <div v-loading="_loading" class="flex flex-col gap-3 min-h-[300px] min-w-[800px] overflow-x-auto">
            <transition name="slide-up" mode="out-in">
                <div :key="activeTab" class="flex flex-col gap-3">
                    ...
                    <div v-for="(group, index) in groupedData" :key="index" class="bg-white border border-gray-200 rounded-[14px] overflow-hidden transition-all duration-300 shadow-sm">
                        <div class="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 transition-colors select-none group" @click.self="toggleAccordion(group.id)">
                            ...
                            <div class="flex items-center gap-4 pointer-events-none">
                                ...
                                <div class="flex items-center gap-5 ml-8">
                                    <span v-for="(stat, idx) in group.stats" :key="idx" class="flex items-center gap-1.5 text-[13px] text-gray-500 whitespace-nowrap">
                            ...
                            <div class="flex items-center gap-3 overflow-hidden">
                                <button @click.stop="openAddModal(group.id)" class="flex items-center gap-1.5 px-4 py-2 bg-[#111827] text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition-all active:scale-95 shadow-sm">
                                    <i class="ri-add-line text-base"></i>
                                    <p class="hidden xl:block line-clamp-1 break-all">{{ group.addText }}</p>
                                </button>
                                <button @click.stop="openEditGroup(group.id)" class="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition-all active:scale-95 shadow-sm">
                        ...
                        <div class="grid-transition-wrapper" :class="{ 'is-open': expandedRows.includes(group.id) }">
                            <div class="overflow-hidden min-h-0 border-t border-gray-100">
                                <div class="w-full">
                                    <table class="w-full text-left border-collapse">
                                        <thead>
                                            <tr class="text-[13px] text-gray-900 border-b border-gray-100 bg-white">
                                                ...
                                                <th class="py-3 px-6 font-semibold w-[10%]">Davomiyligi</th>
...
.slide-up-enter-from { 
    opacity: 0; 
    transform: translateY(20px); /* Yangisi pastdan keladi */
}
.slide-up-leave-to { 
    opacity: 0; 
    transform: translateY(-20px); /* Eskisi tepaga chiqib ketadi */
}
