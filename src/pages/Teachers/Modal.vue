...
const _allSubjects = ref<TeacherSubjectModel[]>([]);
...
function open(item: TeacherModel | null) {
  ...
  if(item) _allSubjects.value = JSON.parse(JSON.stringify(item?.subjects));
}
...
<template>
  <el-dialog v-model="_visible" width="680px" class="teacher-dialog" :show-close="false" append-to-body>
    <template #header>
      <div class="flex items-center justify-between p-1">
        <h3 class="text-lg font-bold text-gray-800">{{ _formData.id ? 'Tahrirlash' : 'Yangi o\'qituvchi' }}</h3>
        <button @click="close" class="w-8 h-8 hover:bg-gray-100 rounded-full transition-all flex items-center justify-center"><i class="ri-close-line text-xl text-gray-400"></i></button>
      </div>
    </template>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <el-form-item label="Fanlar" prop="subjects" class="!mb-4">
          <el-select v-model="_formData.subjects" multiple collapse-tags placeholder="Fanlarni tanlang" class="compact-input w-full" size="default">
            <el-option v-for="s in _allSubjects" :key="s.id" :label="s.name" :value="s.id">
              <div class="flex items-center gap-2"><span>{{ s.emoji || '📖' }}</span><span>{{ s.name }}</span><span class="ml-auto text-xs text-gray-400">{{ s.shortName }}</span></div>
            </el-option>
          </el-select>

          <div v-if="_formData.subjects.length" class="flex flex-wrap gap-2 mt-2">
            <template v-for="id in _formData.subjects" :key="id">
              <div v-if="_allSubjects.find(s => s.id === id)" class="flex items-center gap-1.5 px-2 py-1 rounded-md text-xs border bg-gray-50 border-gray-200" :style="{ color: _allSubjects.find(s => s.id === id)?.color, borderColor: _allSubjects.find(s => s.id === id)?.color + '40', backgroundColor: _allSubjects.find(s => s.id === id)?.color + '10' }">
        ...
        <el-form-item label="Qisqa ism" prop="shortName" class="!mb-2">
          <div class="flex gap-2 w-full">
            ...
            <button type="button" @click="_showAvailability = !_showAvailability" :class="[_showAvailability ? 'bg-green-500 border-green-500 text-white' : 'bg-white text-gray-400 border-gray-200']" class="w-10 h-9 border rounded-lg flex items-center justify-center transition-all hover:shadow-sm"><i class="ri-calendar-todo-line text-lg"></i></button>
          </div>
        </el-form-item>
      </div>
